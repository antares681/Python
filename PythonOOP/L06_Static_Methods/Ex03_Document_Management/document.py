...
class Document:
    ...
    def from_instances(cls, id:int, category:Category, topic:Topic, file_name:str):
        category_id, topic_id = category.id, topic.id
    ...
    def __repr__(self):
        tags_list = ", ".join([tag for tag in self.tags])
        return f"Document {self.id}: {self.file_name}; category {self.category_id}, topic {self.topic_id}, tags: {tags_list}"
